...
const char *WINDOWTITLE = { "Shader project" };
...
float Ssize = 0.5;
float S0 =  1.*(cos(2.*M_PI*0.)); 
float T0 = .1*(cos(2.*M_PI*0.));
float Ds, Dt;
float V0, V1, V2;
float ColorR = 0., ColorG = 1., ColorB = 0.;
float Ka = 0.2, Kd = 0.4, Ks = 0.8;
float Shininess = 10.;
float Time = 0.;
//int MS_PER_CYCLE = 100000;
...
Display()
{
	...
	float plzMov = 1.*(cos(2.*M_PI*Time));
	float plzMov2 = .1*(cos(2.*M_PI*Time));
	printf("plz mov: %d\n", plzMov);
	printf("Time %f\n", Time);
	Pattern->Use();
	Pattern->SetUniformVariable("uS0", S0);
	Pattern->SetUniformVariable("uT0", T0);
	Pattern->SetUniformVariable("uDs", plzMov);
	Pattern->SetUniformVariable("uDt", plzMov2);
	...
	Pattern->SetUniformVariable("uSpecularColor", ColorR, ColorG, ColorB);
	Pattern->SetUniformVariable("uShininess", Shininess);
	Pattern->SetUniformVariable("uSize", Ssize);
	Pattern->SetUniformVariable("uFrozen", frozen);
...
InitLists()
{
	...
	if (!valid)
	{
		printf("YOOOOOOOO IT AINT WORKIN");
	}
